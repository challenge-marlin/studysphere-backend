...
async function checkDatabaseStatus() {
  ...
  try {
    console.log('=== データベース状態確認（修正版） ===');
    
    // 1. ユーザーアカウントの確認
    console.log('\n1. ユーザーアカウント:');
    const [users] = await connection.execute('SELECT id, name, email, role FROM user_accounts LIMIT 5');
    ...
    // 7. 学習開始APIのテスト
    console.log('\n7. 学習開始APIのテスト:');
    if (users.length > 0 && courses.length > 0 && lessons.length > 0) {
      const testUserId = users[0].id;
      const testCourseId = courses[0].id;
      const testLessonId = lessons[0].id;
      
      console.log(`テスト用データ: ユーザーID=${testUserId}, コースID=${testCourseId}, レッスンID=${testLessonId}`);
      
      // コース割り当てのテスト
      try {
        const [assignResult] = await connection.execute(`
          INSERT INTO user_courses (user_id, course_id, status, start_date, progress_percentage, created_at, updated_at)
        ...
        console.log('コース割り当てテスト結果:', assignResult);
      } catch (error) {
        console.log('コース割り当てテストエラー:', error.message);
      }
      ...
      try {
        ...
        console.log('レッスン進捗テスト結果:', progressResult);
      } catch (error) {
        console.log('レッスン進捗テストエラー:', error.message);
      }
    }
