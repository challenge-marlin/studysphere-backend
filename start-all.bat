...
REM 起動監視スクリプトの実行（オプション）
if /I "%MODE%"=="default" (
    echo [INFO] Starting startup monitoring script...
    echo [INFO] Starting startup monitoring script...
    
    REM Node.jsが利用可能かチェック
    node --version >nul 2>&1
    if errorlevel 1 (
        echo [WARN] Node.js not available, skipping startup monitoring
        echo [WARN] Node.js not available, skipping startup monitoring
    ) else (
        REM 起動監視スクリプトを実行
        start "Startup Monitor" cmd /k "cd /d %cd% && node startup-monitor.js"
        echo [INFO] Startup monitoring script started in new window
        echo [INFO] Startup monitoring script started in new window
    )
)
